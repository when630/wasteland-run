    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ color: '#88aabb', fontSize: '32px', margin: 0 }}>명예의 전당</h2>
          <button
            onClick={onClose}
            style={{
        ...
        <div style={{ overflowY: 'auto', flex: 1, paddingRight: '4px' }}>
          {isLoading ? (
            <div style={{ color: '#ccc', textAlign: 'center', padding: '40px', fontSize: '16px' }}>데이터 수신 중...</div>
          ) : leaders.length === 0 ? (
            <div style={{ color: '#ccc', textAlign: 'center', padding: '40px', fontSize: '16px' }}>아직 등록된 기록이 없습니다. 최초의 생존자가 되어보세요!</div>
          ) : (
              ...
              <tbody>
                {leaders.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #334455', backgroundColor: idx % 2 === 0 ? '#1f262c' : 'transparent' }}>
                    ...
                    <td style={{ padding: tdPad, fontWeight: 'bold', maxWidth: '100px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.username}</td>
